    ...
    def _load_from_yaml(self, config_path: Path) -> None:
        ...
        try:
            ...
            if not isinstance(data, dict):
                raise ValueError(f"Invalid languages config format: expected dict, got {type(data)}")
            
            for language, extensions in data.items():
                if not isinstance(extensions, list):
                    logger.warning(f"Invalid extensions for {language}: expected list, got {type(extensions)}")
                    continue
...
class FileScanner(FileScannerInterface):
    ...
    def _should_ignore(self, path: Path, root_path: Path) -> bool:
        ...
        try:
            ...
            if path.is_dir():
                return (
                    self._pathspec.match_file(rel_path_str) or
                    self._pathspec.match_file(rel_path_str + "/")
                )
