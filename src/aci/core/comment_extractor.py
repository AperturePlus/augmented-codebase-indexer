...
class CommentExtractorStrategy(ABC):
    ...
    @abstractmethod
    def extract(
        self, node: Any, content: str, root_node: Any, node_name: Optional[str] = None
    ) -> Optional[str]:
...
class GoDocExtractor(CommentExtractorStrategy):
    ...
    def _first_comment_line_matches(self, comment_lines: List[str], node_name: Optional[str]) -> bool:
...
def extract_jsdoc(
    node: Any, content: str, root_node: Any, node_name: Optional[str] = None
) -> Optional[str]:
