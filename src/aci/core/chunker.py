...
from aci.core.ast_parser import ASTNode, ASTParserInterface
...
class SmartChunkSplitter:
    ...
    def split_oversized_node(
        ...
        context_prefix_tokens = self._tokenizer.count_tokens(context_prefix) if context_prefix else 0
    ...
    def _find_split_points(
        ...
        while current_start < len(lines):
            ...
            if end_idx <= current_start:
                ...
                logger.warning(
                    f"Line {current_start + 1} exceeds token limit, forcing split"
                )
    ...
    def extract(self, content: str) -> List[str]:
        ...
        for line in content.split("\n"):
            ...
            if stripped.startswith("import ") or stripped.startswith("from "):
                ...
                if not any(stripped.startswith(kw) for kw in ["import ", "from ", "#", '"""', "'''"]):
                    break
        return imports
...
class ImportExtractorRegistry:
    ...
    def register(self, language: str, extractor: ImportExtractorInterface) -> "ImportExtractorRegistry":
        """
...
class Chunker(ChunkerInterface):
    ...
    def _chunk_fixed_size(
        ...
        while start_idx < total_lines:
            ...
            while self._tokenizer.count_tokens(chunk_content) > self._max_tokens and len(chunk_lines) > 1:
