...
def search_result(draw):
    ...
    content = draw(st.text(min_size=10, max_size=200, alphabet=st.characters(whitelist_categories=('L', 'N', 'P', 'Z'))))
...
class TestRerankerResultSubset:
    ...
    *For any* search with re-ranking enabled, the Reranker's returned results
    ...
    @given(
        ...
        query=st.text(min_size=5, max_size=100, alphabet=st.characters(whitelist_categories=('L', 'N', 'P', 'Z'))),
    ...
    *For any* search with re-ranking enabled, the Reranker's returned results
    ...
    def test_reranked_results_respect_limit(self, candidates, query, top_k):
        ...
        assert len(results) <= top_k, \
            f"Got {len(results)} results, expected <= {top_k}"
    ...
    def test_reranked_results_return_exactly_top_k_when_enough_candidates(self, candidates, query, top_k):
        ...
        assert len(results) == top_k, \
            f"Got {len(results)} results, expected exactly {top_k}"
    ...
    def test_reranked_empty_candidates_returns_empty(self, query, top_k):
        ...
        assert len(results) == 0, \
            f"Got {len(results)} results for empty candidates, expected 0"
